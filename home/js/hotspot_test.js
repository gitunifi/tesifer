...
function portal(html, hotspotPosition, width, height, leftOrRight, color, resolution, type) {
    ...
    if (clickable !== "null") {
        ...
        var distance = (xDimension / 2) + 10 + corniceLarga;
        ...
        if (leftOrRight === "left") {
            cssObject.translateX(-170);
        } else {
            cssObject.translateX(170);
        }
        cssObject.rotateY(Math.PI*5/16);
...
function makeBordoPannello(width, height, radius, inside, corniceLarga, cornice, color) {
    ...
    var mesh = new THREE.Mesh(geometry, new THREE.MeshLambertMaterial({color: color, opacity: 0.7, transparent: true}));

    mesh.translateX(+width / 2 + cornice + 30);
